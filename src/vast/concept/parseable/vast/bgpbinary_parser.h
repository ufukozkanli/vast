...
struct bgpbinary_parser : parser<bgpbinary_parser> {
  ...
  bool parse_mrt_header(Iterator &f, Iterator const &l, Attribute &a) const {
    ...
    a.timestamp = time::point{time::seconds{t32}};
    // MRT - Type
  ...
  bool parse_bgp4mp_msg_update_withdraw(Iterator &f, Iterator const &l,
    ...
    if ((a.addr_family == 1) & (wd_rts_len > 0)) {
      ...
      while (wd_rts_len > 0) {
        ...
        a.prefix_v4.push_back(
          subnet{address{&t32, address::ipv4, address::host}, wd_prefix_bits});
      }
    ...
    // BGP - Withdraw - IPv6
      ...
      if (a.length != 0) {
        VAST_WARN("Packet-End not reached", a.length);
        a.paket_error = true;
        return false;
      } else
        return true;
  ...
  bool parse_bgp4mp_msg_update_announce(Iterator &f, Iterator const &l,
    ...
    // BGP - Announce - Path Attributes & Network Layer Reachability Information
    // not exist
    if (total_path_len == 0)
      return true;
    ...
    if ((a.addr_family == 1) & (!attr_type_active)) {
      ...
      while (prefix_len > 0) {
        ...
        a.prefix_v4.push_back(
          subnet{address{&t32, address::ipv4, address::host}, prefix_bits});
      }
    }
    ...
    if (a.length != 0) {
      VAST_WARN(
        "The Length is not zero -> there are same not interpreted fields",
        a.length);
      f += a.length;
      return false;
    }
  ...
  format parse(Iterator &f, Iterator const &l, Attribute &a) const {
    ...
    /*MRT Header*/
    if (!parse_mrt_header(f, l, r))
      return {};
    uint32_t length = r.length;
    /*BGP4MP_MESSAGE_AS4*/
    if (!parse_bgp4mp_msg_as4(f, l, r))
      return {};
    /*BGP4MP_STATE_CHANGE*/
    if (parse_bgp4mp_state_change(f, l, r))
      return r;
    /*BGP*/
    if (!parse_bgp(f, l, r))
      return {};
    /*BGP4MP_MESSAGE_UPDATE_WITHDRAW*/
    a.paket_error = false;
    bool parse_rst = parse_bgp4mp_msg_update_withdraw(f, l, r);
    if (parse_rst)
      return r;
    else if ((!parse_rst) & a.paket_error) {
      a.paket_error = false;
      return {};
    }

    /*BGP4MP_MESSAGE_UPDATE_ANNOUNCE*/
    if (parse_bgp4mp_msg_update_announce(f, l, r))
      return r;
    else
      return {};
  }
